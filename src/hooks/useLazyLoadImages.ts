		...
		const observerCallback = (entries: IntersectionObserverEntry[], observer: IntersectionObserver) => {
			entries.forEach((entry) => {
				if (entry.isIntersecting) {
					const dataSrc = entry.target.children.item(0)?.children.item(0)?.getAttribute("data-src");

					if (entry.target.children.item(0)?.getAttribute("src") !== dataSrc && dataSrc) {
						entry.target.children.item(0)?.children.item(0)?.setAttribute("src", dataSrc);
						observer.unobserve(entry.target);
					}

					if (entry.target.children.item(0)?.children.item(0)?.tagName !== "IMG") {
						observer.unobserve(entry.target);
					}
				}
			});
